...
//            overlineMaterial = null;
    ...
    */
    ...
    public BlockFace getLocationsFacing(Location location, Location facingLocation) {
        ...
        if(lZ >= sftlZ && sftlX - (lZ - sftlZ) <= lX && sftlX + (lZ - sftlZ) >= lX) return BlockFace.SOUTH;
        else if(lZ <= sftlZ && sftlX - (sftlZ - lZ) <= lX && sftlX + (sftlZ - lZ) >= lX) return BlockFace.NORTH;
        else if(lX >= sftlX && sftlZ - (lX - sftlX) <= lZ && sftlZ + (lX - sftlX) >= lZ) return BlockFace.EAST;
